<h1 align="center">Hi 👋, I'm A. Yogeshwaran</h1>
<h3 align="center">Aspiring Software Developer | Java Developer | AI Enthusiast</h3>

<p align="center">
  <a href="https://github.com/Developer-Yogeshwaran">
    <img src="https://komarev.com/ghpvc/?username=yourusername&label=Profile%20views&color=0e75b6&style=flat" alt="profile views" />
  </a>
</p>
---
<p align="center">
  <a href="mailto:yogeshofficialwork1@gmail.com">Email</a> • 
  <a href="https://linkedin.com/in/yogeshwaran-a-64655b396">LinkedIn</a> • 
  <a href="https://github.com/Developer-Yogeshwaran">GitHub</a>
</p>

---
## 👨‍💻 About Me

I am a passionate and dedicated Computer Science student from Sethu Institute of Technology.  
I have strong interest in Software Development and Artificial Intelligence.  
I enjoy solving logical problems and building real-world applications.

I am currently seeking internship opportunities to enhance my technical and practical skills.

---


## 🎓 Education

**B.E. Computer Science and Engineering (AI & ML)**  
Sethu Institute of Technology (2023 – 2027)  
CGPA: **8.50**  
Honors in Full Stack Development  

Higher Secondary (Class XII) – 68%  
Secondary (Class X) – 78%

---

## 🛠 Technical Expertise

### 💻 Programming & Development
- Java
- Python
- Node.js
- Object-Oriented Programming (OOP)

### 🤖 Artificial Intelligence
- Machine Learning Concepts
- Natural Language Processing (NLP)
- Generative AI
- AI Chatbot Development
- Resume Parsing & Keyword Extraction

### 🌐 Web & Database
- Frontend Technologies
- MySQL
- Database Management
- RESTful API Development

### ☁️ Tools & Platforms
- Git & GitHub
- VS Code
- Spring Boot
- Cloud Computing (Basics)

---

## 🚀 Project Experience

### 👟 SMART SHOE CHARGER WITH VOICE ASSISTANT  
**AI & IoT Project Developer | July 2025 – Aug 2025**

- Developed an IoT-based wearable integrating renewable energy and health monitoring  
- Implemented self-charging mechanism using sustainable energy concepts  
- Worked on hardware–software integration  
- Demonstrated innovation, teamwork, and real-time problem solving  

---

### 📄 RESUME MATCHER – AI-Based Java Application  
**Backend Developer | June 2025**

- Built AI-powered resume analysis system using **Java & Spring Boot**  
- Implemented text-processing algorithms for:
  - Keyword extraction  
  - Resume scoring  
  - Job description matching  
- Developed REST APIs with basic frontend integration  
- Improved recruitment automation accuracy  

---

### 💬 AI Mental Health Chatbot  
**OpenAI-based NLP Project | July 2025**

- Designed AI chatbot using NLP techniques  
- Handled stress-related and emotional queries  
- Focused on ethical AI usage and user privacy  
- Improved conversational accuracy and response quality  

---

## 📜 Certifications & Training

- 🏆 IBM Certified – Generative AI Fundamentals  
- 🏆 NPTEL – Data Structures and Algorithms using Java  
- 🏆 Infosys Springboard – Networking & Web Technology  
- 🏆 Deloitte Virtual Task Management Program  
- 🏆 AI Course – CodeVibez Learning Hub  
- 🏆 Stellantis Automotive Workshop – NIT Trichy  

---

## 📄 Resume

👉 [Download Resume](./A_Yogeshwaran_Resume.pdf)

---

## 📫 Contact

📧 Email: yogeshofficialwork1@gmail.com  
🔗 LinkedIn: https://linkedin.com/in/yogeshwaran-a-64655b396  
💻 GitHub: https://github.com/Developer-Yogeshwaran

---

⭐ Open to Internship Opportunities in:
- Java Development
- AI / ML
- Backend Development
- Full Stack Development


⭐ Thank you for visiting my profile!
